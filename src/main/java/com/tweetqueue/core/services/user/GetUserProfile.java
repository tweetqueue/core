...
public class GetUserProfile {
  ...
  public UserResponse getUserProfileById(GetUserRequest request) {
    Optional<User> user = userRepository.getById(userFactory.getUserId(request.getId()));

    return userResponseFactory.getUserResponse(
        user.orElseGet(() -> userFactory.getUser(null, null)));
  }
}
